...
public class ApplyLoanUseCase implements IApplyLoanUseCase {
    ...
    public ApplyLoanUseCase(ApplyRepository applyRepository, IUserExtras userExtras, TransactionExecutor transactionExecutor) {
    ...
    public Mono<Apply> saveApply(Apply apply) {
        return transactionExecutor.executeInTransaction(() ->
                userExtras.verifyUser(apply.getNumberIdentity())
                        .flatMap(exists -> {
